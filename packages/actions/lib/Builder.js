...
const log = cli_1._log('Builder')
const safeResolve = (path, config_path) => {
  try {
...
class Builder {
  static async build(
    ...
    const proto_fab = new core_1.ProtoFab()
    ...
    for (const plugin of plugins) {
      ...
      if (Array.isArray(dynamic_runtimes)) {
        log(`Registering additional runtime plugin(s):
          💛${dynamic_runtimes
        ...
        for (const dynamic_runtime of dynamic_runtimes) {
          const path = safeResolve(
            cli_1.relativeToConfig(config_path, dynamic_runtime.runtime),
            config_path
          )
          if (!path) {
    ...
    const typecheck = Typecheck_1.Typecheck.startTypecheck(
      config_path,
      runtime_plugins,
      skip_typecheck
    )
    await Compiler_1.Compiler.compile(config, proto_fab, runtime_plugins, minify)
    await typecheck.waitForResults()
    await Generator_1.Generator.generate(proto_fab)
  }
  static async getPlugins(config_path, config) {
    const plugins = []
    for (const [plugin_name, plugin_args] of Object.entries(config.plugins)) {
      const is_relative = cli_1.isRelative(plugin_name)
      const relative_path = cli_1.relativeToConfig(config_path, plugin_name)
      ...
      if (path_slash_build || path_slash_runtime) {
        if (plugin_path) {
          ...
          log.warn(`For plugin '${plugin_name}', we found ${found_paths.join(
            ' & '
        ...
        if (path_slash_build) {
          ...
          if (!module) {
            throw new cli_1.BuildFailedError(`Error occurred requiring ${path_slash_build}.
