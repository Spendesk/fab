...
import { ENV_VAR_SYNTAX, HOSTING_PROVIDERS } from '@fab/core'
import {
  _log,
  FabDeployError,
  InvalidConfigError,
...
export default class Deployer {
  static async deploy(
    ...
    if (!deploy) {
      throw new FabDeployError(`For the moment, you need to have your fab.config.json5 "deploy" section configured.
  ...
  static getProviders(deploy, server_host, assets_host, skip_assets) {
    ...
    for (const target of targets) {
      ...
      if (!provider) {
        throw new FabDeployError(`Deploy target '${target}' in your fab.config.json5 not supported.
  ...
  static chooseProviderAutomatically(specific_hosts, type, versatile_hosts) {
    ...
    if (specific_hosts.length > 1) {
      throw new InvalidConfigError(`Your fab.config.json5 deploy config has multiple ${type}-only hosts: ${specific_hosts.join(
        ', '
      )}
    ...
    if (versatile_hosts.length > 1) {
      throw new InvalidConfigError(`Your fab.config.json5 deploy config has multiple hosts capable of both server & asset hosting: ${specific_hosts.join(
        ', '
      )}
        Specify which one to use with the --server-host & --assets-host arguments`)
    }
    throw new InvalidConfigError(`Your fab.config.json5 deploy config has no entries for hosts capable of hosting your ${type}.
      See https://fab.dev/guides/deploying for more information.`)
  }
  static resolveEnvVars(config) {
    ...
    if (missing_env_vars.length > 0) {
      throw new InvalidConfigError(`Your deploy config references environment variables that weren't found:
